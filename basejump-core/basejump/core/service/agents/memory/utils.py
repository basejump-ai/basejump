...
"""
    ...
    base_agent = SimpleAgent(prompt_metadata=agent_setup.prompt_metadata, service_context=service_context)
    vis_tool = VisTool(agent=base_agent)
    await vis_tool.get_plot(result_uuid=visual_result.result_uuid, prompt=prompt)
    # Return the new visual result
    assert vis_tool.agent.query_result, "There should be a query result - check your code"
    assert vis_tool.agent.query_result.visual_result_uuid
    return await crud_result.get_visual_result(
        db=vis_tool.agent.db, visual_result_uuid=vis_tool.agent.query_result.visual_result_uuid
    )
...
async def refresh_results(
    ...
    if visual_result:
        visual_result = await refresh_visual_result(visual_result=visual_result)
