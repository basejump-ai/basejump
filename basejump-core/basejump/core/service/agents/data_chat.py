...
class DataChatAgent(BaseChatAgent):
    ...
    async def _refresh_cached_result(
    ...
    async def get_cached_response(self, semcache_response: sch.SemCacheResponse) -> sch.Message:
        ...
        if refresh_not_needed:
            ...
            # Refresh the results
            query_res = await self._refresh_cached_result(result=result, visual_result=visual_result)
    ...
    async def get_cached_prompt(self, prompt: str, distance_threshold: float, num_results=1) -> list[dict[str, Any]]:
        try:
    ...
    async def check_semcache(self, prompt) -> Optional[sch.Message]:
        ...
        semcache_response = await self.get_cached_prompt(
            prompt=prompt, distance_threshold=constants.REDIS_SEMCACHE_EXACT_DISTANCE
        )
